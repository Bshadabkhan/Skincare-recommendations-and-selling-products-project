...
def filter_products(request):
    if request.method == 'POST':
        ...
        if request.POST.get('email'):
            ...
            subject_applicant = 'Wellcome to skin care'
            str=""
            for i in products:
...
def index(request):
    if request.method == 'POST':
        ...
        subject_applicant = f'{name} refered email'
        message_applicant = f'Some one refered, This mail{email}, name of {name}'
        email_from = settings.EMAIL_HOST_USER
        applicant_email = ['sachinuh2001@gmail.com']
        try:
            send_mail(subject_applicant, message_applicant, email_from, applicant_email)
            print('Applicant email sent')
        except Exception as e:
